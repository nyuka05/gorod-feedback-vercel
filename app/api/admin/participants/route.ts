...
export async function POST(request: Request) {
  ...
  await getDatabase().prepare("INSERT INTO participants (id, full_name, project, sort_order, is_active, created_at) VALUES (?, ?, ?, ?, 1, ?)")
...
export async function PATCH(request: Request) {
  ...
  await getDatabase().prepare("UPDATE participants SET full_name = ?, project = ?, sort_order = ?, is_active = ? WHERE id = ?")
...
export async function DELETE(request: Request) {
  ...
  const count = await getDatabase().prepare("SELECT COUNT(*) AS count FROM feedback WHERE recipient_id = ? OR (sender_type = 'participant' AND sender_id = ?)").bind(id, id).first<{ count: number }>();
  if ((count?.count ?? 0) > 0) {
    await getDatabase().prepare("UPDATE participants SET is_active = 0 WHERE id = ?").bind(id).run();
    return Response.json({ ok: true, deactivated: true });
  }
  await getDatabase().prepare("DELETE FROM participants WHERE id = ?").bind(id).run();
