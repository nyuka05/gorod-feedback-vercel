...
type WallData = { participant: { id: string; fullName: string; project: string; totalLikes: number }; messages: WallMessage[] };
    ...
    <main>
      ...
      <section className="shell wall-page">
        ...
        {data && <>
          ...
          <div className="wall-heading"><h2>Поддержка городских продюсеров</h2><Link className="button primary" href={`/write/${data.participant.id}`}>Добавить своё</Link></div>
          {data.messages.length === 0 ? <div className="empty-wall"><span aria-hidden>✦</span><h3>Здесь скоро появятся тёплые слова</h3><p>Можно стать первым, кто поддержит это выступление.</p></div> : (
            <div className="message-grid">{data.messages.map((message, index) => <article className={`message-card tone-${index % 4}`} key={message.id}>
              ...
              <footer><Avatar name={message.senderName || "Команда"} size="small" seed={index + 2} /><div><strong>{message.senderName || "Отправитель"}</strong>{message.senderType === "organizer" && <span>Команда ШГП</span>}</div></footer>
            </article>)}</div>
          )}
        </>}
      </section>
